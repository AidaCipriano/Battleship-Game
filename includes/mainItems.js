...
scene.background = new THREE.Color(0xFEC671);
...
// Crear geometría y material para el "botón" (cubo en este caso)
const buttonGeometry = new THREE.BoxGeometry(2, 1, 0.5); // Ancho, alto y profundidad del cubo
const buttonMaterial = new THREE.MeshBasicMaterial({ color: 0x3498db });
const button = new THREE.Mesh(buttonGeometry, buttonMaterial);
button.scale.set(1, 1, 1);
button.position.set(10, 2, 0); // Posición en el espacio 3D

// Crear geometría y material para el "botón" (cubo en este caso)
const buttonGeometry2 = new THREE.BoxGeometry(2, 1, 0.5); // Ancho, alto y profundidad del cubo
const buttonMaterial2 = new THREE.MeshBasicMaterial({ color: 0x3498db });
const button2 = new THREE.Mesh(buttonGeometry2, buttonMaterial2);
button2.scale.set(1, 1, 1);
button2.position.set(10, 7, 0); // Posición en el espacio 3D
...
document.addEventListener('click', (event) => {
    ...
    // Dispara un rayo desde la cámara al punto donde hizo clic el usuario
    const raycaster = new THREE.Raycaster();
    raycaster.setFromCamera(mouse, camera);

    // Obtiene una lista de objetos que intersectan con el rayo
    const intersects = raycaster.intersectObjects([button]); // Solo intersecta con el botón
    const intersects2 = raycaster.intersectObjects([button2]); // Solo intersecta con el botón

    // Si el botón fue clicado, cambia el color
    if (intersects.length > 0) {
        cambiarColor();
        Otroturno = true;
        //cambiarTurno();
        console.log('agarraste el item');
    ...
    if (intersects2.length > 0) {
        cambiarColor2();
        Dosturnos = true;
        console.log('agarraste el item2');
    }
});
...
const cambiarColor = () => {
    ...
    // Aplica el nuevo color al material del botón
    button.material.color = nuevoColor;
    $('#Mas1').modal('show');
...
const cambiarColor2 = () => {
    ...
    // Aplica el nuevo color al material del botón
    button2.material.color = nuevoColor;
    $('#Mas2').modal('show');
...
const materialPlane = new THREE.MeshBasicMaterial({ color: 0x2A007A, wireframe: true });
...
const materialPlane02 = new THREE.MeshBasicMaterial({ color: 0x2A007A, wireframe: true });
...
const matPlane = new THREE.MeshBasicMaterial({ color: 0xB22600, wireframe: true });
...
const materialPlaneE02 = new THREE.MeshBasicMaterial({ color: 0xB22600, wireframe: true });
...
function cambiarTurno() {
    ...
    if (Otroturno == true) {
        console.log('Has activado un item');
    ...
    //if (Dosturnos == true) { 
        while(Dosturnos == true) {
        console.log('Has activado un item 2');
        desactivarEventosJugador2();
        activarEventosJugador1();
        contador++;
